    ...
    public static void main(String[] args) {
        ...
        for (int packetSize : packetSizes) {
            if (currentBucketSize + packetSize <= bucketCapacity) {
                ...
                System.out.println(
                        packetSize + "\t\t" + currentBucketSize + "\t\t" + Math.min(outputRate, currentBucketSize)
                                + "\t\t" + Math.max(0, currentBucketSize - outputRate) + "\t\tAccepted");
            } else {
                System.out.println(
                        packetSize + "\t\t" + currentBucketSize + "\t\t" + Math.min(outputRate, currentBucketSize)
                                + "\t\t" + Math.max(0, currentBucketSize - outputRate) + "\t\tDropped");
            }
